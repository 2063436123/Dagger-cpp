...
static std::string trans_level_to_str(Logger::LogLevel level) {
...
static void prepend_msg_for_test(string &msg, Logger::LogLevel level) {
    struct timeval tv;
    ...
    string header = buf + to_string(tv.tv_usec) + " " + trans_level_to_str(level) + "] ";
    msg = header + msg;
}
...
void Logger::log(LogLevel level, const char *logline, size_t len) {
    ...
    prepend_msg_for_test(msg, level);
...
void Logger::backThread() {
    ...
    // 如果在收回buffers_后有新的数据被写入currentBuffer_，并且随后Logger::stop()被调用，那么while()判断为false，缓冲区中数据就会丢失.
    // 此处实现可以补救currentBuffer_和buffers_中的数据
